...
def import_empirical_data_wrapper(path_data, path_subjecttracker, data_load_param, import_responses_fn=None, import_participants_fn=None, calc_filter_criteria_fn=None, package_fn=None, followup_fn=None, plot_param=None, bypass_plotting=False, debug=False):
    ...
    for criteria, fn in filter_fn_dict.items():
        ...
        ### temp hack for saved pickles because pandas no longer excepts lambda x: x in ['correct_response, '-1'] for exp 9 data
        ### def fn(x): return (x == 'correct_response' or x == '-1')
    ...
    if datasheet_participants['randCondNum'].abs().max() + 1 != data_load_param['ncond'] or datasheet_participants['randCondNum'].abs().unique().size != data_load_param['ncond']:
        warnings.warn(f"WARNING:: randCond numbers: {datasheet_participants['randCondNum'].abs().max()+1}    {datasheet_participants['randCondNum'].abs().unique().size-1}     {data_load_param['ncond']}")
        # print(datasheet_participants['randCondNum'].abs().unique())
        print(f"{datasheet_participants['randCondNum'].abs().max()+1}")
        print(len(datasheet_participants['randCondNum'].abs().unique()))
        print(data_load_param['ncond'])
    ...
    # print(f">> Total passing validation exclusion:{exclusion} filter:{subjRespFilter} (exp 11): {np.sum(datasheet_participants['subjectIncluded'])}, min,max cond val: {mmin} / {mmax}\n")
    # print(f">> Total passing validation (exp 11): {np.sum(datasheet_participants['subjectIncluded'])}, min,max cond val: {mmin} / {mmax}\n")
...
def make_nobs_df(datasheet_in, outcomes=None, pots=None):
    ...
    index_outcome = np.full((len(outcomes), datasheet_in.shape[0]), False, dtype=np.bool)
    index_pot = np.full((len(pots), datasheet_in.shape[0]), False, dtype=np.bool)
    ...
    #     ppldata['pots_byoutcome'+suffix][outcome] = pots_temp # np.unique( alldata.loc[ (alldata[('stimulus','outcome')] == outcome), ('stimulus','pot') ] )
...
def make_invplan_nobs_df(datasheet_in, a1_list=None, pots=None):
    ...
    nobsdf = pd.DataFrame(data=nobsarray, index=pots, columns=a1_list, dtype=np.int64)
    nobsdf.index.set_names(['pots'], inplace=True)
...
def import_responses_exp10(path_data, data_stats, data_load_param=None):
    ...
    # stimulus='all', condition=None, emoLabels=ppldata['labels']['emotions'], data_load_param={**data_load_param
...
def import_participants_exp7(path_subjecttracker, data_load_param=None):
    ...
    datasheet_participants = pd.read_csv(path_subjecttracker,
                                         ...
                                         converters={"randCondNum": lambda x: np.nan if x == 'NaN' else x},
                                         )
...
def package_empdata_InversePlanning_exp6_exp9(df_wide, feature_list, shorthand, shorthand_list, a1_labels):
    ...
    empiricalPreferenceJudgments['rider_hack'] = {'df_wide': df_wide, 'feature_list': feature_list, 'shorthand': shorthand, 'shorthand_list': shorthand_list}  # DEBUG
...
def importEmpirical_exp3_7_11_(cpar, game_full, game_exp3, verbose=False):
    from webpypl_importjson import importPPLmodel_, importEmpirical_
    import copy
    import numpy as np
    ...
    ### load exp7 exp11
    ppldata, wpplparam = importPPLmodel_(game_full.data_path_, game_full.wpplparam, game_full.pots, verbose)
    """
    modelDataPath=game_full.data_path_
    wpplparam=game_full.wpplparam
    pots=game_full.pots
    """
    data_stats_7_11 = importEmpirical_exp7_11_(ppldata, cpar)

    ### load exp3
    importEmpirical_(ppldata, paths['exp3xlsx'], paths['subjectrackerexp3'], verbose, suffix='_exp3')

    ### load exp3
    ppldata_exp3, wpplparam_exp3 = importPPLmodel_(game_exp3.data_path_, game_exp3.wpplparam, game_exp3.pots, verbose, suffix='')
    importEmpirical_(ppldata_exp3, paths['exp3xlsx'], paths['subjectrackerexp3'], verbose, suffix='')

    intersect, exp7, exp3 = list(), list(), list()
    for feature in ppldata['labels']['emotions']:
        intersect.append(feature) if feature in ppldata_exp3['labels']['emotions'] else exp7.append(feature)
    for feature in ppldata_exp3['labels']['emotions']:
        if feature not in ppldata['labels']['emotions']:
            exp3.append(feature)
    print('Intersecting emotion labels: ')
    print(intersect)
    print('Only in exp7: ')
    print(exp7)
    print('Only in exp3: ')
    ...
    ppldata['labels']['emotions_lo_intersect' + '_exp3'] = [feature for feature in ppldata['labels']['emotions'] if feature in ppldata_exp3['labels']['emotions']]
    ppldata_exp3['labels']['emotions_lo_intersect'] = copy.copy(ppldata['labels']['emotions_lo_intersect' + '_exp3'])

    ### make df for emotions that overlap in the empirical datasets
    empiricalEmotionJudgments_intersect = {'nobs': ppldata_exp3['empiricalEmotionJudgments']['nobs']}
    for i_outcome, outcome in enumerate(ppldata_exp3['labels']['outcomes']):
        tempdf = ppldata_exp3['empiricalEmotionJudgments'][outcome].loc[:, (slice(None), ppldata_exp3['labels']['emotions_lo_intersect'] + ['prob'])]
        empiricalEmotionJudgments_intersect[outcome] = tempdf.reindex(columns=zip(['emotionIntensities'] * len(ppldata_exp3['labels']['emotions_lo_intersect']) + ['prob'], ppldata_exp3['labels']['emotions_lo_intersect'] + ['prob']))

    ppldata['empiricalEmotionJudgments_intersect' + '_exp3'] = empiricalEmotionJudgments_intersect
    ppldata_exp3['empiricalEmotionJudgments_intersect'] = empiricalEmotionJudgments_intersect

    ppldata_exp3['prospect_fn'] = ppldata['prospect_fn']
    ppldata_exp3['potspacing'] = ppldata['potspacing']

    return ppldata, ppldata_exp3, wpplparam
...
def importEmpirical_InversePlanning_exp6_exp9(cpar, level, bypass_plotting=False):
    ...
    ### df_wide, feature_list, shorthand, shorthand_list
