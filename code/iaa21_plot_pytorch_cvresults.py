...
def new_emo_plot_iaf(ppldataemodf, emoevdf=None, scale_factor=1.0, bandwidth=0.05, emotions_abbriv=None, xrotate=True, fig_outpath=None, plotParam=None):
...
def format_iaf_labels_superphrase_original(iaf_labels):
...
def format_iaf_labels_superphrase(iaf_labels):
    ...
    iaf_labels_formatted = list()
    for label in iaf_labels:
        label1 = label.replace('U[', 'AU[').replace('a1[', '{a1}[').replace('a2[', '{a2}[').replace('[base', '^{\mathrm{base}}[').replace('[repu', '^{\mathrm{repu}}[').replace('PEa2lnpot', 'PE[~\pi_{a2{=}\\texttt{C}}]').replace('[Money]', '_{\\mathit{Money}}').replace('[DIA]', '_{\\mathit{DIA}}').replace('[AIA]', '_{\\mathit{AIA}}').replace('[', '').replace(']', '')
        labelf = r"$" + label1 + r"$"
        iaf_labels_formatted.append(labelf)
...
def format_iaf_labels_superletter(iaf_labels):

    iaf_labels_formatted = list()
    for label in iaf_labels:
        label1 = label.replace('U[', 'AU[').replace('a1[', '_{a1}[').replace('a2[', '_{a2}[').replace('[base', '^{~\mathbf{b}~}[').replace('[repu', '^{~\mathbf{r}~}[').replace('PEa2lnpot', 'PE[~\pi_{a2{=}\mathrm{C}}]').replace('[', ' ').replace(']', '')
        labelf = r"$" + label1 + r"$"
        iaf_labels_formatted.append(labelf)
...
def composite_Aweights(learned_param_Amean=None, emo_labels=None, iaf_labels=None, fig_outpath=None, plotParam=None):
    ...
    ax = sns.heatmap(A_, annot=True, linewidths=.5, center=0, vmin=-1 * maxval, vmax=maxval, cmap=cmap_, annot_kws={"size": 5}, fmt=".1f", ax=ax, cbar=False, )  # )mask=mask_) ax.set_facecolor("lightblue")
...
def single_Aweights(learned_param_A=None, emotion=None, emo_labels=None, iaf_labels=None, xrange=None, cialpha=None, fig_outpath=None, plotParam=None):
    ...
    for i_feature, feature in enumerate(iaf_labels):
        ...
        if np.min(xxxci) <= 0 and np.max(xxxci) >= 0:
            ...
            # sns.diverging_palette(200, 300, s=75, l=40, sep=2, center='light', as_cmap=True)
            if np.min(xxxci) > 0:
