...
RUN conda create --name envcaa --file /imgbase/conda-linux-aarch64.lock
RUN conda clean --all --yes

RUN echo ". ${CONDA_DIR}/etc/profile.d/conda.sh && conda activate envcaa" >> /etc/skel/.bashrc
RUN echo ". ${CONDA_DIR}/etc/profile.d/conda.sh && conda activate envcaa" >> ~/.bashrc
...
ENTRYPOINT ["/opt/conda/envs/envcaa/bin/python"]